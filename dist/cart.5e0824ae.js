...
var cartPro = JSON.parse(localStorage.getItem('key')) || [];
var prise = document.querySelector('#pris');
var dipcon = document.querySelector('#disp');
var tot = 0;
var mer = JSON.parse(localStorage.getItem('ke'));
function displayList() {
  console.log(cartPro);
  cartPro.forEach(function (element) {
    var divs = " \n        <div class=\"container col-10 mt-5 border d-flex\">\n        <div class=\"row col-md-2 col-sm-4 col-4 p-2 text-center justify-content-center\">\n          <img src=\"".concat(element.img, "\" class=\"img-fluid p-2 rounded shadow \" alt=\"Product Image\">\n        </div>\n        <div class=\"col-md-4 col-9 mt-2 ms-2 p-2\">\n          <h2>").concat(element.name, "</h2>\n          <p><strong>Price: $</strong>").concat(element.prize, "</p>\n          <p>Quantity : 1</p>\n          <button onclick=\"remove(").concat(element.id, ")\">remove</button>\n        </div>\n    </div>");
...
function remove(proId) {
  mer--;
  var pro = cartPro.map(function (p) {
    return p.id;
  });
  var index = pro.findIndex(function (item) {
    return item === proId;
  });
  console.log(pro);
  if (index !== -1) {
...
function total() {
  var tik = cartPro.map(function (cur) {
    return cur.prize++;
  ...
  if (tik == Error) {
    ...
    prise.innerHTML = "$".concat(Math.floor(tik));
  }
}
;
function name() {
  var ver = cartPro.length;
